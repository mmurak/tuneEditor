...
class ToneNote {
  ...
  getPath() {
    ...
    if (spath.length > 0) {
      ...
      let magnifier = 1.0
      if (maxdev != 0) {
        magnifier = (lowerLimit - this.height) / maxdev;
      }
      for(let i = 0; i < spath.length; i++) {
        let x = this.width + margin + (spath[i][0] * this.magX);
...
class Separator {   // could be subclass of sth... like Element
  ...
  draw(context, margin) {
    context.beginPath();
    context.moveTo(this.width+margin, upperLimit);
    context.lineTo(this.width+margin, lowerLimit);
    context.stroke();
  }
}
